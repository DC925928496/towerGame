...
class GameSaveModel(BaseModel):
    ...
    def _validate_field_types(self) -> List[str]:
        ...
        if self.save_name is not None:
            if not isinstance(self.save_name, str):
                errors.append("存档名称必须是字符串")
        # is_active 类型验证
        if self.is_active is not None:
            if not isinstance(self.is_active, int) or isinstance(self.is_active, bool):
                errors.append("是否为当前激活存档必须是整数")
        # created_at 类型验证
        if self.created_at is not None:
            if not isinstance(self.created_at, datetime):
                errors.append("存档创建时间必须是日期时间对象")
        # updated_at 类型验证
        if self.updated_at is not None:
            if not isinstance(self.updated_at, datetime):
                errors.append("存档更新时间必须是日期时间对象")
        return errors
    ...
    def _validate_field_constraints(self) -> List[str]:
        ...
        if self.floor_level is not None:
            ...
            if self.floor_level < 0:
                errors.append("存档时所在楼层不能为负数")
        # save_name 约束验证
        if self.save_name is not None:
            # 字符串长度检查
            if len(self.save_name) > 200:
                errors.append("存档名称长度不能超过200个字符")
        # is_active 约束验证
        if self.is_active is not None:
            # 数值范围检查
            if self.is_active < 0:
                errors.append("是否为当前激活存档不能为负数")
        # created_at 约束验证
        if self.created_at is not None:
            # datetime类型不需要长度验证
            pass
    ...
    def _clean_data(self):
        ...
        if self.save_name:
            # 去除首尾空格
            self.save_name = self.save_name.strip()
        # is_active 数据清理
        if self.is_active is not None:
            try:
