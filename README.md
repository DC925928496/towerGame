...
- ✅ **重复内容移除**：清理README.md中的重复描述，优化文档结构
- ✅ **代码质量提升**：移除无用注释和调试代码，提高代码可读性
- ✅ **git状态优化**：清理未跟踪的临时文件，优化版本控制状态
- ✅ **词条数值展示优化**：非百分比类词条统一进行数值舍入，默认按整数展示，少数强化/重铸场景最多保留两位小数（百分比属性统一保留一位小数），彻底避免出现 2.98999999 这类长小数
- ✅ **商人防具属性修复**：商人出售的防具补全稀有度与词条列表，购买并装备后属性与界面展示保持一致

**📦 历史更新：v2.11 - 商人界面与交互优化**
...
- ✅ **代码清理**：移除了旧版商人标签页/快捷键相关样式与脚本

**📦 历史更新：v2.9 - 防具词条系统完整实现**
- ✅ **防具词条系统**：完整实现9个防具核心词条，与武器系统形成混合词条体系
- ✅ **防具属性类型**：防御强化、伤害减免、荆棘反射、格挡、闪避、生命加成、上楼回血、击杀回血、药水增效
- ✅ **四档稀有度系统**：普通(1词条)、稀有(2词条)、史诗(3词条)、传说(4词条)，支持数值加成
- ✅ **动态防具生成**：智能防具名称生成系统，根据主属性自动生成独特名称
- ✅ **战斗效果集成**：完整的防具战斗反馈系统，包括伤害减免、格挡、闪避等详细日志
- ✅ **数据库扩展**：新增equipment_type字段，支持武器防具统一存储，实现完整持久化
- ✅ **前端界面优化**：防具卡片式显示，支持词条列表和稀有度颜色，与武器系统UI统一
- ✅ **平衡设计**：60%防御属性 + 40%生存属性的合理权重分布，符合Roguelike游戏机制

**📦 历史更新：v2.8 - 武器系统大升级与界面优化**
- ✅ 武器词条扩展：新增6个中低优先级词条，从9个扩展到15个完整武器属性体系
...
- ✅ 伤害计算机制优化：防御力现在��以完全抵消伤害，提升装备策略价值
...
### v2.8 代码清理优化 🧹
- **演示文件清理**：删除weapon_card_demo.html演示文件，功能已集成到主游戏
- **导入语句优化**：移除game_server.py中未使用的asdict导入，减少代码冗余
- **空行格式优化**：压缩代码中过多的空行，提升代码紧凑性和可读性
- **项目结构整洁**：保持56个Python文件和1个HTML文件的精简结构
- **编译验证通过**：所有Python文件通过语法检查，确保代码质量
...
### 商人交互系统（v1.5 纯键盘版）
> 注：以下内容记录旧版全键盘交互，当前版本已采用鼠标+E/ESC模式
...
### 数据库连接优化 🔧
- **智能重试机制**：连接失败时自动重试2次，采用递增等待时间（2秒、4秒）
- **超时时间优化**：连接超时从5秒增加到10秒，提高连接成功率
- **连接池调优**：优化连接池大小和超时配置，减少资源消耗
- **优雅降级**：数据库不可用时自动切换到本地存档模式
- **安全操作包装**：新增数据库操作辅助函数，提供更好的错误处理和回退机制

### 前端界面优化 🎨
- **帮助弹窗系统**：新增完整帮助弹窗，包含图例、操作说明和游戏机制讲解
- **卡片式设计**：武器展示采用现代化卡片布局，支持不同稀有度的视觉区分
...
*核心特色：用户认证 + 完整存档系统 + 自动保存优化 + 15个武器词条支持 + 9个防具词条支持 + 混合词条体系 + 智能代码生成工具 + 企业级实体类系统 + 游戏经济系统重平衡 + 帮助弹窗系统 + 数据库智能重试 + 代码库清理优化 + Roguelike平衡设计*
*版本：v2.13 - 药瓶机制重构（小/中/大三档百分比回血） + 装备掉落逻辑优化（每层最多1把武器与1件防具） + 跨平台部署支持（Linux + Nginx + WebSocket /ws） + 本地调试体验改进（Windows file:// + ws://localhost:8080） + 无用代码清理（旧版药瓶兼容逻辑移除）*
